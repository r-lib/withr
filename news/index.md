...
- Fixed issue that prevented
  [`local_seed()`](https://withr.r-lib.org/reference/with_seed.md) from
  preserving the seed
...
- [`defer()`](https://withr.r-lib.org/reference/defer.md) is now a thin
  wrapper around
  ...
  [`defer()`](https://withr.r-lib.org/reference/defer.md) is now much
  faster (although still slower than
  ...
  [`defer()`](https://withr.r-lib.org/reference/defer.md) with
  ...
  versions of [`defer()`](https://withr.r-lib.org/reference/defer.md).
...
- When [`source()`](https://rdrr.io/r/base/source.html) is used with a
  ...
  [`defer()`](https://withr.r-lib.org/reference/defer.md) or `local_`
  functions at top-level of a script). This support is disabled by
  default in local environments to avoid a performance penalty in normal
  usage of withr features.

### Other features and bugfixes

- [`deferred_run()`](https://withr.r-lib.org/reference/defer.md) now
  reports the number of executed expressions with a message.

- [`deferred_run()`](https://withr.r-lib.org/reference/defer.md) can now
  be run at any point in a knitr file
...
- [`local_pdf()`](https://withr.r-lib.org/reference/devices.md) and
  friends now correctly restore to the previously active device
  ([\#138](https://github.com/r-lib/withr/issues/138)).

- [`local_()`](https://withr.r-lib.org/reference/with_.md) now works
  even if withr isn’t attached
  ([\#207](https://github.com/r-lib/withr/issues/207)).

- [`local_par()`](https://withr.r-lib.org/reference/with_par.md) and
  [`with_par()`](https://withr.r-lib.org/reference/with_par.md) now work
...
- [`defer()`](https://withr.r-lib.org/reference/defer.md) and all
  `local_*()` functions now work when run inside of a `.Rmd`. The
  deferred expressions are executed when knitr exits.

- [`defer()`](https://withr.r-lib.org/reference/defer.md) and `local_`
  functions now work within
...
- [`with_()`](https://withr.r-lib.org/reference/with_.md) and
  [`local_()`](https://withr.r-lib.org/reference/with_.md) gain a `get`
  argument. Supply a getter function to create `with` and `local`
...
- [`local_rng_version()`](https://withr.r-lib.org/reference/with_rng_version.md)
  and [`local_seed()`](https://withr.r-lib.org/reference/with_seed.md)
  no longer warn when restoring `sample.kind` to `"Rounding"`
  ([\#167](https://github.com/r-lib/withr/issues/167)).

- [`with_seed()`](https://withr.r-lib.org/reference/with_seed.md) now
  preserves the current values of
...
- Handlers registered with the global environment (as happens when
  [`local_()`](https://withr.r-lib.org/reference/with_.md) is run at the
  top-level, outside a function) are now automatically run when the R
  session ends ([\#173](https://github.com/r-lib/withr/issues/173)).
...
- [`with_seed()`](https://withr.r-lib.org/reference/with_seed.md) now
  caches the check for R version, so is now faster
...
- [`with_seed()`](https://withr.r-lib.org/reference/with_seed.md) now
  also sets the RNG via new arguments `.rng_kind`, `.rng_normal_kind`
  and `.rng_sample_kind`
...
- Tests for
  [`with_cairo_pdf()`](https://withr.r-lib.org/reference/devices.md) and
  [`with_cairo_ps()`](https://withr.r-lib.org/reference/devices.md) have
  been removed, as they fail if Cairo is not available, such as with M1
  macOS systems ([\#158](https://github.com/r-lib/withr/issues/158))

- [`local_seed()`](https://withr.r-lib.org/reference/with_seed.md) is
  now exported ([\#153](https://github.com/r-lib/withr/issues/153))
...
- New [`local_seed()`](https://withr.r-lib.org/reference/with_seed.md)
  function and
  [`local_preserve_seed()`](https://withr.r-lib.org/reference/with_seed.md)
  functions to correspond to
  [`with_seed()`](https://withr.r-lib.org/reference/with_seed.md) and
...
- [`defer()`](https://withr.r-lib.org/reference/defer.md) now throws an
  error if an error occurs in the deferred expression
  ([\#148](https://github.com/r-lib/withr/issues/148))

- [`with_file()`](https://withr.r-lib.org/reference/with_file.md) and
  [`local_file()`](https://withr.r-lib.org/reference/with_file.md) can
  now work if the file is actually a directory
...
- [`defer()`](https://withr.r-lib.org/reference/defer.md) can set
  deferred events on `.GlobalEnv` to facilitate the interactive
  development of code inside a function or test. Helpers
  [`deferred_run()`](https://withr.r-lib.org/reference/defer.md) (and
...
- [`set_makevars()`](https://withr.r-lib.org/reference/set_makevars.md)
