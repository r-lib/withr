...
- [`with_dir()`](https://withr.r-lib.org/reference/with_dir.md): working
  directory
...
- [`with_sink()`](https://withr.r-lib.org/reference/with_sink.md):
  output redirection

## Creating new "with" functions

All `with_` functions are created by a helper function,
[`with_()`](https://withr.r-lib.org/reference/with_.md). This functions
accepts two arguments: a setter function and an optional resetter
function. The setter function is expected to change the global state and
return an "undo instruction". This undo instruction is then passed to
the resetter function, which changes back the global state. In many
cases, the setter function can be used naturally as resetter.
